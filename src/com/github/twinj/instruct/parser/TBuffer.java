...
/**
 * {@link SeekableByteChannel} implementation backed by an auto-resizing byte
 * array; thread-safe. Can hold a maxiumum of {@link Integer#MAX_VALUE} bytes.
 * 
 * @author <a href="mailto:alr@jboss.org">Andrew Lee Rubinger</a>
 */
public class TBuffer implements SeekableByteChannel {
	...
	/**
	 * Whether or not this {@link SeekableByteChannel} is open; volatile instead
	 * of sync is acceptable because this field participates in no compound
	 * computations or invariants with other instance members.
	 */
	...
	public static TBuffer newTBuffer(final byte[] src) {
		TBuffer buffer = new TBuffer();
		
		try {
			buffer.write(ByteBuffer.wrap(src));
		} catch (IOException ex) {}
		
		return buffer;
	}
	
	public static TBuffer newTBuffer(ByteBuffer src) {
		TBuffer buffer = new TBuffer();		
		try {
			buffer.write(src);
		} catch (IOException ex) {}		
	...
	public int read(final ByteBuffer destination) throws IOException {
		...
		synchronized (this) {
			...
			// We'll read in either the number of bytes remaining in content or the
			// amount of space in the buffer,
			// whichever is smaller
			numBytesToRead = numBytesRemainingInContent >= spaceInBuffer ? spaceInBuffer
						: numBytesRemainingInContent;
	...
	/**
	 * Creates a new array which is the concatenated result of the two inputs, at
	 * the designated position (to be filled with 0x00) in the case of a gap).
	 * 
	 * @param input1
	 * @param input2
	 * @param position
	...
	private byte[] concat(final byte[] input1, final byte[] input2, final int position) {
		...
		// Allocate a new array of enough space (either current size or position +
		// input2.length, whichever is greater)
		final int newSize = position < input1.length ? input1.length + input2.length
	...
	public SeekableByteChannel position(final long newPosition) throws IOException {
		...
		if (newPosition > Integer.MAX_VALUE || newPosition < 0) {
			throw new IllegalArgumentException(
						"Valid position for this channel is between 0 and " + Integer.MAX_VALUE);
		}
		synchronized (this) {
	...
	public SeekableByteChannel truncate(final long size) throws IOException {
		...
		if (size < 0 || size > Integer.MAX_VALUE) {
			throw new IllegalArgumentException("This implementation permits a size of 0 to "
		...
		synchronized (this) {
			...
			// If the current position is greater than the given size, set to the
			// given size (by API spec)
	...
	/**
	 * Throws a {@link ClosedChannelException} if this {@link SeekableByteChannel}
	 * is closed.
	 * 
